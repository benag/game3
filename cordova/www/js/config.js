/**
 * Created by ben on 16/09/15.
 */

var config = {
    global: {
        inside:'cordova',
        registerSound: 'cordova',
        compileWith: 'cordova'
    },
    assets: {
        BLUBALOON: 'img/BlueB.png',
        GREENBALOON: 'img/GreenB.png',
        REDBALLOON: 'img/RedB.png',
        BACKGROUNDURL: 'img/BG_1.jpg',
        FILTERBACKURL: 'img/blueLayer.png',
        EXPLOSIONURL:  'img/bubbles.png',
        UPPERSCORE: 'img/upper-score.png',
        NEGATIVESCORE: 'img/scoringminus5.png',
        POSITIVESCORE: 'img/scoring20.png',
        CHILDIMAGE: 'img/boy.png',
        TITLEII: 'img/title.png',
        INSTRUCTBIT: 'img/text_button.png',
        STOPMUSIC: 'img/Sound-off-icon.png',
        BUTTOMLINE: 'img/botttom-score.png',
        UPPERLINE: 'img/upper-score.png',
        SCORE20LARGE: 'img/score20Large.png',
        STAR: 'img/star.png',
        START: 'img/start.png'

    }

};